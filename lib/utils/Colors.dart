import 'dart:ui';

class AppColors {
  static final Color textColor = const Color(0xFFccc7c5);
  static final Color mainColor = const Color(0xFFFF0000);

  //static final Color mainColor = const Color(0xFFfa4552);
  static final Color iconColor1 = const Color(0xFFffd28d);
  static final Color iconColor2 = const Color(0xFFfcab88);
  static final Color paraColor = const Color(0xff8f837f);
  static final Color buttonBackgroundColor = const Color(0xffeaeaea);
  static final Color signColor = const Color(0xFFa9a29f);
  static final Color titleColor = const Color(0xFF5c524f);
  static final Color mainBlackColor = const Color(0xFF000000);
  static final Color blueColor = const Color(0xFF0D47A1);
  static final Color mainBlueColor = const Color(0xFF64B5F6);

  //static final Color yellowColor = const Color(0xFF332d2b);
  static final Color yellowColor = const Color(0xFFffd379);
}
